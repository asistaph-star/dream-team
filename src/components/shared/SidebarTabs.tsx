...
export interface SidebarTab {
  ...
  /** Short label for horizontal/narrow mode */
...
export function SidebarTabs({
  ...
  return (
    <>
      {/* Vertical sidebar: visible on md+ screens */}
      <div 
        className={`bg-[#2a2b2f] flex-col shrink-0 relative z-10 hidden md:flex ${className}`}
        ...
        {tabs.map(tab => {
          ...
          return (
            <button
              key={tab.id}
              onClick={() => onTabChange(tab.id)}
              className={`flex-1 min-w-0 px-2 py-3 text-[11px] font-bold tracking-wide text-center transition-all whitespace-nowrap ${
                isActive
                  ? 'text-white border-b-2 border-[#ff7300] bg-white/5'
