...
export function MatchStage() {
  ...
  // Clock state countdown and interpolation
  ...
  useEffect(() => {
    ...
    const duration = 1200 / MATCH_PLAYBACK_SPEED;
    const steps = 24;
  ...
  const renderPlayerCard = (p: Player, isAi: boolean, slotPos?: string, isGhost = false) => {
    ...
    const halfWidth = 1420 / 2;
    
    let spacedLeft = 0;
    if (!isAi) {
      const leftBoundary = pad;
      const rightBoundary = halfWidth - playerWidth - 10;
      spacedLeft = leftBoundary + (index * (rightBoundary - leftBoundary)) / 4;
    } else {
      const leftBoundary = halfWidth + 10;
      const rightBoundary = 1420 - playerWidth - pad;
      spacedLeft = leftBoundary + (index * (rightBoundary - leftBoundary)) / 4;
    }
    
    const t = Math.max(0, Math.min(1, (1200 - 1420) / 500)); // 0 since 1420 is fixed
    const rawLeft = origLeft + t * (spacedLeft - origLeft);
    const finalLeft = Math.max(15, Math.min(1420 - 15 - playerWidth, rawLeft));

    const minY = 110 + 10;
    const maxY = 800 - 210 - playerHeight - 10;
  ...
  const matchStyles = getMatchStyles(1);
  ...
  if (viewState === 'SIMULATING' || viewState === 'HALFTIME') {
    return (
      <div className="absolute inset-0 w-full h-full font-sans bg-black overflow-hidden select-none pointer-events-none">
        ...
        {/* Global Ghost Drag Image (fixed positioning tracking cursor) */}
        {draggingPlayerId && (
          <DragGhost
        ...
        {/* 1. GAME WORLD COURT LAYER (statically centered on stage) */}
        <div 
          className="stadium absolute shadow-2xl pointer-events-auto"
          style={{
            left: '58px',
            top: '32px',
            width: '1420px',
        ...
        >
            ...
            {/* CLUTCH / HALFTIME NARRATIVE OVERLAY */}
            {viewState === 'HALFTIME' && (
              <div className="absolute inset-0 bg-black/75 backdrop-blur-md flex flex-col items-center justify-center z-[150] animate-[fadeIn_0.2s_ease-out]">
                <SkewedBadge className="mb-4" innerClassName="border-yellow-500 bg-yellow-500/10 px-8 py-2">
                  <span className="text-yellow-400 font-black tracking-widest text-lg italic">HALFTIME REPORT</span>
                </SkewedBadge>
                <div className="text-center max-w-lg mb-6">
                  <div className="text-3xl font-black text-white mb-2">{matchState.userScore} - {matchState.aiScore}</div>
                  <p className="text-gray-300 text-xs italic">
                    {matchState.userScore > matchState.aiScore ? 'Solid performance in the first half! Hold the lead and secure the win.' : matchState.userScore < matchState.aiScore ? 'We are trailing at the break. Make some defensive adjustments and rally!' : 'Tied game! The second half will decide the victor.'}
                  </p>
                </div>
                <button 
                  onClick={handleHalftimeResume} 
                  className="px-8 py-3 rounded-lg font-black text-xs uppercase tracking-wider bg-yellow-500 text-black hover:bg-yellow-400 active:scale-95 transition-all shadow-[0_4px_15px_rgba(234,179,8,0.4)] cursor-pointer"
                >
                  RESUME GAME ({halftimeCountdown}s)
                </button>
              </div>
            )}
        </div>
