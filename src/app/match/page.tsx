  ...
  const renderPlayerCard = (p: Player, isAi: boolean, slotPos?: string, isGhost = false) => {
    ...
    return (
      ...
      >
        ...
        {/* ═══ PREMIUM SLANTED BANNERS (DTPH Style — 100% Transparent, English, Dynamic Premium AI Silhouettes) ═══ */}
        {(() => {
          /* ─── CONFIGURE BANNER DATA DYNAMICALLY ─── */
          let bannerData: {
            label: string;
            count: string;
            bg: string;
          ...
          if (showFtPopup) {
            const ftOutcome = matchState.ftSequence!.results[activeFtIndex];
            if (ftOutcome === 'make') {
              bannerData = {
                label: 'FT MADE',
                count: '+1',
            ...
            const is3 = matchState.lastPointsScored === 3;
            const eventText = matchState.events[0]?.text?.toLowerCase() || '';
            let label = 'PTS';
            let count = '+2';
            
            if (eventText.includes('dunk')) {
              label = 'PTS';
              count = '+2';
            } else if (is3 || eventText.includes('three') || eventText.includes('deep') || eventText.includes('downtown')) {
              label = 'PTS';
              count = '+3';
            } else {
              label = 'PTS';
            ...
            bannerData = {
              ...
              textColor: '#ffffff',
              countColor: '#ffedd5',
              silhouetteImg: eventText.includes('dunk') ? '/silhouette_dunk.png' : '/silhouette_shoot.png',
              silhouetteFilter: 'none',
            };
          } else if (justMissed) {
            bannerData = {
          ...
          } else if (matchState.lastEventIndicator?.playerId === p.id) {
            const ev = matchState.lastEventIndicator!;
            switch (ev.type) {
              case 'AST':
                bannerData = {
                  label: 'ASSIST',
                  count: '+1',
                  // Defensive / Possession helper is unified Blue theme
                  ...
                  textColor: '#ffffff',
                  countColor: '#dbeafe',
                  silhouetteImg: '/silhouette_pass.png',
                  silhouetteFilter: 'none', // Already a gorgeous glowing blue chest-pass pose
                };
                break;
              case 'STL':
                bannerData = {
                  ...
                  silhouetteFilter: 'none', // Beautiful electric cyan 2-player steal battle
                };
                break;
              case 'REB':
              case 'OREB':
                bannerData = {
                  ...
                  textColor: '#ffffff',
                  countColor: '#dbeafe',
                  silhouetteImg: '/silhouette_rebound.png',
                  silhouetteFilter: 'none', // Electric blue 2-player rebound battle under glass
                };
                break;
              case 'BLK':
                bannerData = {
                  ...
                  textColor: '#ffffff',
                  countColor: '#dbeafe',
                  silhouetteImg: '/silhouette_block.png',
                  silhouetteFilter: 'none', // Purple/magenta 2-player shot-rejection battle
                };
                break;
              case 'TOV':
                bannerData = {
                  label: 'TURNOVER',
                  count: '',
                  // Turnover/Foul is unified Gray/Red theme
                  bg: 'linear-gradient(95deg, rgba(28,25,23,0.95) 0%, rgba(68,64,60,0.95) 60%, rgba(87,83,78,0.95) 100%)',
                  border: 'rgba(239,68,68,0.5)',
                  glow: 'rgba(239,68,68,0.45)',
                  textColor: '#fca5a5',
                  countColor: '#ef4444',
                  silhouetteImg: '/silhouette_steal.png',
                  silhouetteFilter: 'grayscale(1) brightness(0.55) sepia(1) hue-rotate(-45deg) saturate(3)', // Dark red
                  topLine: '#ef4444',
                };
                break;
              case 'FOL':
          ...
          return (
            <div 
              ...
              style={{ 
                animation: `${showFtPopup ? 'banner-pop 0.55s' : 'banner-pop 1.5s'} cubic-bezier(0.25, 1, 0.5, 1) forwards`,
                height: '38px',
              }}
            >
              {/* Centering wrapper */}
        ...
        {matchState.ftSequence && matchState.ftSequence.shooterId === p.id && (() => {
          ...
          return (
            <div className="absolute -top-20 left-1/2 -translate-x-1/2 z-50 flex flex-col items-center gap-1 bg-[#0c0c0e]/95 border-2 border-yellow-500 rounded-xl px-3 py-1.5 shadow-[0_0_20px_rgba(234,179,8,0.5)] animate-bounce select-none pointer-events-none whitespace-nowrap">
              <span className="text-[8px] font-black text-yellow-400 tracking-widest uppercase">{ft.isAnd1 ? 'AND-1 FT' : 'FREE THROW'}</span>
                  ...
                  if (i < activeFtIndex) {
                    ...
                    return (
                      <div key={i} className="relative flex items-center justify-center w-6 h-6">
                        {r === 'make' ? (
                          <span className="text-xl filter drop-shadow-[0_0_6px_rgba(34,197,94,1)]" style={{ textShadow: '0 0 10px rgba(34,197,94,0.8)' }}>🏀</span>
                        ) : (
                    ...
                    if (activeFtStatus === 'aiming') {
                      ...
                      return (
                        <div key={i} className="relative flex items-center justify-center w-6 h-6">
                          <div className="text-xl relative animate-bounce flex items-center justify-center" style={{ animationDuration: '0.4s' }}>
                            🏀
                      ...
                      return (
                        <div key={i} className="relative flex items-center justify-center w-6 h-6">
                          {r === 'make' ? (
                            <span className="text-xl filter drop-shadow-[0_0_8px_rgba(16,185,129,1)] scale-110 duration-200" style={{ textShadow: '0 0 12px rgba(16,185,129,0.9)' }}>🏀</span>
                          ) : (
        ...
        {/* ═══ NBA 2K-STYLE CURVED ARC SHOT METER OVERLAY ═══ */}
              ...
              <path
                ...
                stroke={
                  shotMeterStatus === 'release'
                    ? (matchState.activeShotMeter.isSuccess ? '#10b981' : '#ef4444')
                    : '#facc15'
                }
                strokeWidth="8"
                strokeLinecap="round"
                ...
                style={{
                  filter: shotMeterStatus === 'release' 
                    ? (matchState.activeShotMeter.isSuccess ? 'url(#glow-green)' : 'url(#glow-red)')
                    : 'none'
                }}
              />
        ...
        {matchState.activeShotMeter && matchState.activeShotMeter.playerId === p.id && shotMeterStatus === 'release' && (
          <div className="absolute inset-0 z-[200] pointer-events-none select-none flex flex-col items-center justify-center">
            {matchState.activeShotMeter.isSuccess ? (
              <>
    ...
    return (
      <div className="flex w-full h-full justify-center items-center font-sans bg-black overflow-hidden fixed inset-0 z-[100]">
        ...
        <div className="stadium">
            ...
            {/* SCORING RUN OVERLAY — screen shake + CSS lightning + text only */}
            {(() => {
              ...
              const label = isUser ? 'MY TEAM' : aiTeam.name.toUpperCase();
