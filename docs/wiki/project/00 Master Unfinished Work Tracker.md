...
- Phase ResponsiveAppLayoutSystem-BatchA: Completed and verified. Added responsive game viewport scale system, cover scaling for game court art, safe overlay layouts for UI/HUD, collapsible drawers for reserves on mobile, collapsible chat component, clamped cards in visible bounds, and pure CSS centered BottomNav.
- Phase ResponsiveGameViewportSafeOverlay: Completed and verified. Added CSS variables for viewport margins and bottom nav safe areas, decoupled player cards and scoreboard scaling from court crop, stacked bottom panels vertically below 768px on the Match page, collapsed match chat box to a toggleable icon below 1024px, auto-collapsed strategy and lobby chat panel details below 900px, and visually clamped cards safely without state changes.

Next phases:
- Phase ResponsiveAppLayoutSystem-BatchB: Planned. Responsive layouts for Player Storage, Inventory, Modal responsiveness polish, and shared responsive wrappers.
